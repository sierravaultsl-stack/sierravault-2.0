...
export default function DashboardPage() {
  ...
  const normalizedDocuments = userDocuments.map(doc => ({
    ...
    // @ts-ignore
    label: doc.title || doc.label || doc.type, // Map title (new) or label (old)
    uploadDate: (doc as any).createdAt || doc.uploadedAt,
    url: doc.url,
    mimeType: (doc as any).metadata?.mimeType || doc.type,
    status: (doc as any).status || "pending",
    visibility: "private", // Default for now
    txId: (doc as any).txId || "TX_" + doc._id?.toString().slice(-6).toUpperCase(),
    onChainHash: (doc as any).metadata?.fileHash || (doc as any).onChainHash,
    aiScore: (doc as any).metadata?.aiScore || (doc as any).aiScore || 0.98
  }))
    ...
    <div className="p-4 lg:p-8 space-y-8">
      ...
      <div>
        <h1 className="text-2xl font-bold text-foreground lg:text-3xl">
          Welcome back{user.ninRecord?.name ? `, ${user.ninRecord.name}` : user.email ? `, ${user.email.split("@")[0]}` : ""}
        </h1>
