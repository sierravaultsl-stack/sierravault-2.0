...
async function seed() {
    ...
    // Create Vaults first
    const adminVault = await Vault.create({ userId: new mongoose.Types.ObjectId() });
    const citizenVault = await Vault.create({ userId: new mongoose.Types.ObjectId() });

    const hashedPassword = await bcrypt.hash("password123", 10);

    console.log("Creating Gov Admin...");
