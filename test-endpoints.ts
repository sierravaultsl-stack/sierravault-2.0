...
async function login(email, password) {
    ...
    try {
        ...
        throw new Error(`Failed to parse JSON. Status: ${res.status}. Response: ${text.substring(0, 1000)}...`);
    }

    if (!res.ok) throw new Error(`Login failed: ${data.error} (Status: ${res.status})`);
